...
public:
	...
	static std::vector<vec2> collision_points(Polygon& a, Polygon& b);
...
public:	
	...
	std::vector<vec2> collision_points(Polygon& b);
	float distance(Polygon& b);

	// Return a coefficiet [0, 1] of vec2 motion that the polygon could achieve without 
	// colliding with polygon b.
	float move_until(vec2 motion, Polygon& b);

	// Member access
	vec2& pos() { return pos_; }
	float radius() { return radius_; }
	std::vector<vec2>& joints() { return joints_; }
