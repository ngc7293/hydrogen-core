...
public:
	...
	static std::vector<vecf> collision_points(Polygon& a, Polygon& b);
...
public:
	...
	std::vector<vecf> collision_points(Polygon& b);
	float distance(Polygon& b);

	// Return a coefficiet [0, 1] of vecf motion that the polygon could achieve without 
	// colliding with polygon b.
	float move_until(vecf motion, Polygon& b);

	// Member access
	vecf& pos() { return pos_; }
	float radius() { return radius_; }
	std::vector<vecf>& joints() { return joints_; }
